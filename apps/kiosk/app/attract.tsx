...
// Idle timeout: show attract screen again after 90 seconds of inactivity
// (This screen IS the attract screen — used as the kiosk idle/home)
...
// Gradient cycle: base colour + overlay accent
const GRADIENT_CYCLES = [
  { base: '#0d0820', overlay: '#c2410c' },  // deep purple → orange
  { base: '#07130d', overlay: '#15803d' },  // dark green
  { base: '#0a0e1a', overlay: '#1d4ed8' },  // midnight → blue
  { base: '#120a00', overlay: '#b45309' },  // dark → amber
];
...
export default function AttractScreen() {
  ...
  // Idle timeout ref (reset on any interaction)
  const idleTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  // Track timestamp when app went to background so we can detect long absences
  const backgroundTimestamp = useRef<number | null>(null);

  const resetIdleTimer = useCallback(() => {
    if (idleTimer.current) clearTimeout(idleTimer.current);
    // If we're not on the attract screen already and 90s pass, navigate back
    // (The attract screen itself doesn't need to do much — it's already idle)
  ...
  useEffect(() => {
    ...
    // AppState: if app returns to foreground after IDLE_TIMEOUT_MS, reset to attract
      ...
      if (nextState === 'background' || nextState === 'inactive') {
        ...
        if (backgroundTimestamp.current !== null) {
          ...
          if (elapsed >= IDLE_TIMEOUT_MS) {
            // App was gone long enough — treat as idle, reset to attract
            resetOrder();
            router.replace('/attract');
            return;
          }
        }
        // Resumed before timeout — reset the idle timer
        resetIdleTimer();
      }
    }
  ...
  // Cycle gradient colours every 8 seconds
  useEffect(() => {
