...
const ORG_ROLES: Array<'support' | 'superadmin'> = ['support', 'superadmin'];
...
export default function OrgAccountsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setAccounts(data.data.filter((s) => ORG_ROLES.includes(s.role as 'support' | 'superadmin')));
    } catch (err) {
  ...
  async function handleDeactivate(id: string) {
    ...
    try {
      ...
      setActionError(err instanceof Error ? err.message : 'Failed to deactivate account.');
    }
  }
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white">Organisation Portal Accounts</h1>
          <p className="text-gray-500 text-sm mt-1">Manage support staff who can log into the Organisation Portal</p>
        </div>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#1e1e2e]">
              ...
              <th className="px-6 py-3 text-left text-xs text-gray-500 uppercase tracking-wider">Active</th>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-8 text-center text-gray-600">
                  No organisation portal accounts found
                </td>
              </tr>
            ) : (
                  ...
                  <td className="px-6 py-3">
                    <span
                      className={`px-2 py-0.5 rounded text-xs font-medium ${
                        ROLE_COLORS[account.role] ?? 'bg-gray-500/20 text-gray-400'
                      }`}
                    >
                      {account.role}
                    </span>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-[#111118] border border-[#1e1e2e] rounded-lg p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-white font-semibold">Add Organisation Portal Account</h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  value={form.role}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, role: e.target.value as 'support' | 'superadmin' }))
                  }
                  className="w-full bg-[#0a0a0f] border border-[#1e1e2e] rounded px-3 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500"
                >
