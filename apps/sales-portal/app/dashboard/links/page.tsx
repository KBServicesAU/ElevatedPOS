...
import { Plus, Copy, Check, X, ExternalLink, AlertCircle } from 'lucide-react';
...
export default function LinksPage() {
  ...
  // Ref for focus trap / first focusable element in modal
  const modalFirstInputRef = useRef<HTMLSelectElement>(null);
  ...
  async function copyLink(code: string, id: string) {
    ...
    try {
      ...
      // Clipboard API unavailable (non-HTTPS / permissions denied)
      setCopyFailedId(id);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">My Signup Links</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage and share your referral signup links</p>
        </div>
        <button
          onClick={openModal}
      ...
      {fetchError && (
        <div role="alert" className="flex items-center gap-3 rounded-xl bg-red-50 dark:bg-red-900/40 border border-red-200 dark:border-red-700 px-4 py-3 text-sm text-red-700 dark:text-red-300">
          ...
          <button
            onClick={fetchLinks}
            className="ml-auto text-red-600 dark:text-red-300 underline hover:text-red-500 dark:hover:text-red-200 text-xs"
          >
            Retry
          </button>
        </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {sortedLinks.map((link) => (
                  <tr key={link.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors">
                    ...
                    <td className="px-5 py-3 text-sm text-gray-500 dark:text-gray-500">
                      {link.usedByOrgId ? (
                        <span className="font-mono text-xs text-gray-500 dark:text-gray-400">{link.usedByOrgId.slice(0, 8)}...</span>
                      ) : (
                        '---'
                      )}
                    </td>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors border ${
                            copyFailedId === link.id
                              ...
                              : 'bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white border-gray-200 dark:border-gray-700'
                          }`}
                        >
                          {copiedId === link.id ? (
                            <>
                        ...
                        <a
                          ...
                          className="inline-flex items-center p-1.5 rounded-md text-gray-400 dark:text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                        >
      ...
      {showModal && (
        ...
        >
          <div
            ...
            onClick={closeModal}
          />
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-2xl w-full max-w-md p-6">
            {/* Modal header */}
            <div className="flex items-center justify-between mb-5">
              <h2 id="modal-title" className="text-lg font-semibold text-gray-900 dark:text-white">Create Signup Link</h2>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleCreateLink} className="space-y-4">
              ...
              <div>
                <label htmlFor="plan-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Plan <span className="text-gray-400 dark:text-gray-500">(optional)</span>
                </label>
                <select
                  id="plan-select"
                  ...
                  className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent [color-scheme:dark]"
                >
              ...
              <div>
                <label htmlFor="org-name" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Org Name Hint <span className="text-gray-400 dark:text-gray-500">(optional)</span>
                </label>
                <input
                  id="org-name"
                  type="text"
                  ...
                  className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                />
              </div>

              {/* Note */}
              <div>
                <label htmlFor="link-note" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none"
                />
              </div>

              {/* Expiry date */}
              <div>
                <label htmlFor="expires-at" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              ...
              <div className="flex gap-3 pt-1">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 py-2.5 px-4 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-lg transition-colors text-sm border border-gray-200 dark:border-gray-700"
                >
                  Cancel
                </button>
                <button
