...
import { getDeviceToken, getDeviceInfo, fetchWithDeviceAuth, getAdminPin, setAdminPin, type DeviceInfo } from '@/lib/device-auth';
...
// ─── App switcher bar (with PIN gate) ────────────────────────────────────────
      ...
      <div className="flex items-center gap-1">
        {APPS.map((app) => {
          ...
          return (
            <button
              key={app.id}
              onClick={() => !active && onNavRequest(app.href)}
              className={`flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${
                active ? 'bg-[#2a2a2a] text-white cursor-default' : 'text-gray-600 hover:bg-[#2a2a2a] hover:text-gray-200'
              }`}
            >
...
function AttractScreen({ onStart }: { onStart: () => void }) {
  return (
    ...
    >
      ...
      <h1 className="mb-3 text-5xl font-extrabold tracking-tight text-white">Order Here</h1>
...
function CartSidebar({
  cart, onRemove, onAdd, onCheckout, isLoading,
}: {
  cart: CartItem[];
  ...
  return (
    <div className="flex w-72 flex-col border-l border-[#222] bg-[#0a0a0a] xl:w-80">
        ...
        <div className="flex-1 space-y-2 overflow-y-auto p-3">
          {cart.map((item) => (
            <div key={item.id} className="flex items-center gap-2 rounded-xl border border-[#222] bg-[#111] p-2.5">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => onRemove(item.id)} className="flex h-6 w-6 items-center justify-center rounded-full bg-[#222] text-white hover:bg-[#333]">
                  <X className="h-3 w-3" />
                </button>
                <span className="w-5 text-center text-sm font-bold text-white">{item.qty}</span>
                <button onClick={() => onAdd(item.id)} className="flex h-6 w-6 items-center justify-center rounded-full bg-amber-500 text-black hover:bg-amber-400">
      ...
      <div className="border-t border-[#222] p-4">
        ...
        >
          {isLoading ? <><div className="h-5 w-5 animate-spin rounded-full border-2 border-black border-t-transparent" /> Placing order…</> : 'Pay Now'}
        </button>
      </div>
    </div>
  );
...
function CheckoutSuccess({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 bg-[#0a0a0a] text-center">
      ...
      <button onClick={onNew} className="mt-4 rounded-2xl bg-amber-500 px-10 py-4 text-lg font-extrabold text-black hover:bg-amber-400">
        New Order
      </button>
    </div>
  );
...
function KioskTerminal({ deviceInfo }: { deviceInfo: DeviceInfo | null }) {
      ...
      try {
        ...
        if (productsRes.ok) {
          ...
          const mapped: Product[] = apiProducts.map((p) => ({
            id: p.id, name: p.name, price: p.price / 100,
            category: (p.categoryId && categoryMap[p.categoryId]) ?? p.categoryName ?? 'Other',
            tags: [], emoji: '🛒', description: p.description ?? '', ageRestricted: false,
          }));
          setProducts(mapped);
        }
      } catch {
  ...
  const increaseQty = (id: string) => { const p = products.find((x) => x.id === id); if (p) addToCart(p); };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      await fetch('/api/kds', {
        ...
        body: JSON.stringify({
          type: 'new_order', paymentMethod: 'kiosk',
          order: {
            orderId: `kiosk-${Date.now()}`, orderNumber,
            orderType: 'dine_in', channel: 'kiosk', locationId,
  ...
  const handleNew = () => {
    setCart([]); setCheckedOut(false); setStarted(false); setCategory('All'); setSearch('');
  };

  // Admin PIN navigation handler
  const handleNavRequest = useCallback((href: string) => {
    setPendingNav(href);
    setPinError(null);
  }, []);
    ...
    <div className="relative flex h-full flex-col bg-black">
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`flex-shrink-0 rounded-full px-5 py-2 text-sm font-semibold transition-colors ${
                    category === cat ? 'bg-amber-500 text-black' : 'bg-[#1a1a1a] text-gray-400 hover:text-white'
                  }`}
                >
                  {cat}
                </button>
                    ...
                    >
                      <div className="relative mb-2 flex h-16 items-center justify-center rounded-xl bg-[#111]">
                        ...
                        {product.ageRestricted && (
                          <span className="absolute right-1 top-1 rounded-md bg-red-600 px-1.5 py-0.5 text-[9px] font-bold text-white">18+</span>
                        )}
                        {inCart && (
                      ...
                      {product.tags.filter(Boolean).length > 0 && (
                        <div className="mb-1.5 flex flex-wrap gap-1">
                          {product.tags.filter(Boolean).map((tag) => (
                            <span key={tag} className="rounded-md border border-green-800 bg-green-950/40 px-1.5 py-0.5 text-[9px] font-bold text-green-400">{tag}</span>
                          ))}
                        </div>
                      )}
                      <p className="mb-1 font-bold leading-tight text-white">{product.name}</p>
                      <p className="mb-2 line-clamp-2 text-[11px] leading-tight text-gray-600">{product.description}</p>
      ...
      {pendingNav && (
        <div className="absolute inset-0 z-50">
          <PinPad
            title="Admin Access"
            subtitle="Enter admin PIN to navigate away"
            length={4}
            onSubmit={handlePinSubmit}
            onCancel={() => { setPendingNav(null); setPinError(null); }}
            error={pinError}
            confirmLabel="Unlock"
          />
        </div>
...
// ─── Page — device pairing gate → admin PIN setup → kiosk ────────────────────
...
export default function KioskPage() {
  ...
  // Admin PIN setup flow (only runs once until PIN is set)
  if (!adminPinSet) {
    if (!confirmPin) {
      return (
        <PinPad
          title="Set Admin PIN"
          subtitle="Choose a 4-digit PIN to lock kiosk navigation"
          length={4}
          onSubmit={(pin) => setConfirmPin(pin)}
          error={pinSetupError}
          confirmLabel="Next →"
        />
      );
    ...
    return (
      <PinPad
        ...
        onCancel={() => { setConfirmPin(null); setPinSetupError(null); }}
        error={pinSetupError}
