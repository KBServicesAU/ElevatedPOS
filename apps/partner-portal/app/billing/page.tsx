      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">March 2026 billing period</p>
          </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-slate-700">Current Billing Period — March 2026</h2>
              ...
              <tbody className="divide-y divide-slate-100">
                {BILLING_ROWS.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-mono text-slate-800">${row.monthlyFee.toLocaleString()}</td>
                    <td className="px-5 py-3.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${STATUS_STYLES[row.status]}`}>
                        {row.status}
                      </span>
                    </td>
                    <td className="px-5 py-3.5 text-slate-500">{row.invoiceDate}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                <tr>
                  <td className="px-5 py-3.5 font-semibold text-slate-700" colSpan={2}>Total</td>
                  <td className="px-5 py-3.5 font-bold text-slate-900 font-mono">${total.toLocaleString()}</td>
                  <td colSpan={2} />
                </tr>
              </tfoot>
            </table>
