    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-8 py-4">
          ...
          <p className="text-sm text-slate-500">Manage your partner account preferences</p>
        </header>
        ...
        <div className="p-8 space-y-6 max-w-3xl">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <h2 className="text-sm font-semibold text-slate-700">Partner Profile</h2>
              <p className="text-xs text-slate-400 mt-0.5">Your business details shown on invoices</p>
            </div>
            <div className="p-6 grid grid-cols-2 gap-4">
              ...
              <div className="col-span-2">
                <label className="block text-xs font-medium text-slate-600 mb-1.5">Business Address</label>
                <input
                  type="text"
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <h2 className="text-sm font-semibold text-slate-700">Revenue Share</h2>
              <p className="text-xs text-slate-400 mt-0.5">Commission rates are set by your partnership agreement</p>
            </div>
            <div className="divide-y divide-slate-100">
              {COMMISSION_RATES.map((r) => (
                <div key={r.plan} className="px-6 py-4 flex items-center justify-between">
                  <div>
                    <span className="text-sm font-medium text-slate-800">{r.plan} plan</span>
                    <span className="ml-2 text-xs text-slate-400">{r.price}</span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-bold text-indigo-600">{r.rate}</span>
                    <p className="text-xs text-slate-400">{r.amount}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-400 mt-0.5">Choose which events trigger email notifications</p>
            </div>
            <div className="divide-y divide-slate-100">
              {[
                ...
                <div key={key} className="px-6 py-4 flex items-center justify-between">
                  ...
                  <button
                    ...
                    className={`relative w-10 h-5.5 rounded-full transition-colors ${notifications[key] ? 'bg-indigo-600' : 'bg-slate-200'}`}
                    style={{ height: '22px', width: '40px' }}
                    aria-pressed={notifications[key]}
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-4.5 h-4.5 bg-white rounded-full shadow transition-transform ${notifications[key] ? 'translate-x-4.5' : ''}`}
                      style={{
                        width: '18px',
                        height: '18px',
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100">
              <h2 className="text-sm font-semibold text-slate-700">API Credentials</h2>
              <p className="text-xs text-slate-400 mt-0.5">Use this key to authenticate partner API requests</p>
            </div>
            <div className="p-6">
              <label className="block text-xs font-medium text-slate-600 mb-1.5">Partner API Key</label>
              <div className="flex items-center gap-2">
                <div className="flex-1 flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg font-mono text-sm text-slate-700">
                ...
                <button className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-800 transition-colors">
                  <RefreshCw className="w-3.5 h-3.5" />
                  Regenerate
                </button>
              </div>
              <p className="text-xs text-slate-400 mt-2">Last used 2 days ago. Keep this key secure — do not share it publicly.</p>
            </div>
          </div>
